 ...
 */
function parseSourceAwareAnswer(rawContent) {
        ...
        sources: [
            {
                ...
                note: "โมเดลไม่ได้ตอบกลับเป็น JSON ตามรูปแบบที่กำหนด จึงไม่สามารถแยกแหล่งที่มาได้",
                confidence: "low"
            }
        ],
        source_warning: "ไม่สามารถตรวจสอบแหล่งที่มาของคำตอบได้จาก response นี้"
    };
}
/**
 ...
 */
function printAnswerSources(sources, warning) {
    ...
    sources.forEach((source, index) => {
        ...
        console.log(`Referenced Text : ${source.quoted_or_referenced_text ?? "N/A"}`);
 ...
 */
async function askOpenRouter(userInput) {
    const messages = [
        {
            role: "system",
    ...
    if (!response.ok) {
        return {
            ...
            source_warning: "API request ไม่สำเร็จ จึงไม่มีคำตอบจากโมเดลให้ตรวจสอบแหล่งที่มา"
        };
    }
